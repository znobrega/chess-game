
import java.awt.Color;
import javax.swing.JPanel;

/**
 * Interface Grafica do Tabuleiro do jogo.
 * 
 * @author José Carlos Alves da Nóbrega Júnior &lt;josenobrega@cc.ci.ufpb.br&gt;
 * @author Alan Moraes &lt;alan@ci.ufpb.br&gt;
 * @author Leonardo Villeth &lt;lvilleth@cc.ci.ufpb.br&gt;
 */
public class TabuleiroGUI extends JPanel {

    private JanelaPrincipal janela;
    private CasaGUI[][] casas;

    /**
     * Creates new form Tabuleiro
     */
    public TabuleiroGUI() {
        // Construtor sem par�metros requerido pela especifica�?o JavaBeans.
    }

    public TabuleiroGUI(JanelaPrincipal janela) {
        this.janela = janela;
        initComponents();
        criarCasas();
    }

    /**
     * Preenche o tabuleiro com 64 casas
     */
    private void criarCasas() {
        casas = new CasaGUI[8][8];
        // De cima para baixo
        for (int i = 7; i >= 0; i--) {
            // Da esquerda para a direita
            for (int j = 0; j < 8; j++) {
                Color cor = calcularCor(i, j);
                CasaGUI casa = new CasaGUI(i, j, cor, this);
                casas[i][j] = casa;
                add(casa);
            }
        }
    }

    private Color calcularCor(int i, int j) {
        // linha par
        if (i % 2 == 0) {
            // coluna �mpar
            if (j % 2 == 0) {
                return CasaGUI.COR_CLARA;
            }
            // coluna �mpar
            else {
                return CasaGUI.COR_ESCURA;
            }
        }
        // linha �mpar
        else {
            // coluna par
            if (j % 2 == 0) {
                return CasaGUI.COR_ESCURA;
            }
            // coluna �mpar
            else {
                return CasaGUI.COR_CLARA;
            }
        }

        // c�digo acima em uma linha
        // return (i%2 + j%2)%2 == 0 ? CasaGUI.COR_CLARA : CasaGUI.COR_ESCURA;
    }

    public void atualizar(Jogo jogo) {
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                CasaGUI casaGUI = casas[i][j];
                
                Tabuleiro tabuleiro = jogo.getTabuleiro();
                Casa casa = tabuleiro.getCasa(i, j);
                if (casa.possuiPeca()) {
                    Peca peca = casa.getPeca();

                    switch (peca.getTipo()) {
                        case Peca.KING_BLACK: 
                            casaGUI.colocarKingBlack();
                            break;
                        case Peca.KING_WHITE: 
                            casaGUI.colocarKingWhite();
                            break;
                        case Peca.QUEEN_BLACK: 
                            casaGUI.colocarQueenBlack();
                            break;
                        case Peca.QUEEN_WHITE: 
                            casaGUI.colocarQueenWhite();
                            break;
                        case Peca.TOWER_BLACK: 
                            casaGUI.colocarTowerBlack();
                            break;
                        case Peca.TOWER_WHITE: 
                            casaGUI.colocarTowerWhite();
                            break;
                        case Peca.BISHOP_BLACK: 
                            casaGUI.colocarBishopBlack();
                            break;
                        case Peca.BISHOP_WHITE: 
                            casaGUI.colocarBishopWhite();
                            break;
                        case Peca.KNIGHT_BLACK: 
                            casaGUI.colocarKnightBlack();
                            break;
                        case Peca.KNIGHT_WHITE: 
                            casaGUI.colocarKnightWhite();
                            break;
                        case Peca.PAWN_BLACK: 
                            casaGUI.colocarPawnBlack();
                            break;
                        case Peca.PAWN_WHITE: 
                            casaGUI.colocarPawnWhite();
                            break;
                    }
                }
                else {
                    casaGUI.removerPeca();
                }
            }
        }
    }

    public JanelaPrincipal getJanela() {
        return janela;
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        setLayout(new java.awt.GridLayout(8, 8));
    }// </editor-fold>//GEN-END:initComponents

    // Variables declaration - do not modify//GEN-BEGIN:variables
    // End of variables declaration//GEN-END:variables
}
